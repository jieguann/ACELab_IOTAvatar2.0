...
namespace MeshTracer
{
	public class VertexParticles : MonoBehaviour {
		...
		public int particlesPerVertex = 10; // number of particles allowed to be emitted from each vertex
		public bool randStartPos = false; // start each wave at a random position

		private float particleTime = 0;// particleTime = particleLifetime/particlesPerVertex. = time we should wait in between emitting perticles per vertex
		...
		private GameObject particleSystem; // instance of 'particleSystemPrefab'
		...
		void Start () 
		{
			...
			particleTime = particleLifetime/particlesPerVertex;
			...
			emittedParticledBuffer = new float[someVerticies.Count,Mathf.Max(1,numTracers),1]; // we have to have at least 1 space for the numtracers column
			for(int i = 0; i < someVerticies.Count; i++)
			{
				for(int t = 0; t < numTracers; t++)
				{
					emittedParticledBuffer[i,t,0] = 0;
				}
			}

			//instantiate the prefab
			particleSystem = Instantiate(particleSystemPrefab,this.transform.position,this.transform.rotation) as GameObject;
		...
		public void OrderVerticies()
		{
			...
			case Type.WAVE_RIGHT:
				...
				Vector3[] temp = verticies.OrderBy(vertex => vertex.x).ToArray();
			...
			case Type.WAVE_LEFT:
				//compile a list of ordererd verticies, by -x
				temp = verticies.OrderBy(vertex => -vertex.x).ToArray();
			...
			case Type.WAVE_UP:
				...
				temp = verticies.OrderBy(vertex => vertex.y).ToArray();
				tempVerticies = new List<Vector3>();
				for(int i = 0; i < temp.Length; i++)
				{
					tempVerticies.Add(temp[i]);
			...
			case Type.WAVE_FORWARD:
				...
				temp = verticies.OrderBy(vertex => vertex.z).ToArray();
				tempVerticies = new List<Vector3>();
				for(int i = 0; i < temp.Length; i++)
				{
					tempVerticies.Add(temp[i]);
		...
		IEnumerator SparkleVerticies()
		{
			...
			ps.simulationSpace = ParticleSystemSimulationSpace.World;// use world space
			...
			while(playEffect)
			{
				...
				if(waveAnimation.play && objectVisible )
				{
					...
					float evaluateIndex = animationTimer/waveAnimation.animTime; // % along our animation
					float sparkleValue = waveAnimation.animCurve.Evaluate(evaluateIndex); // evaulate the animation curve at 'evaluateIndex'
					...
					for(int i = 0; i < someVerticies.Count; i++)
					{
						...
						Vector3 pos = transform.TransformPoint(verticies[index]);
						if(cameraCulling.perVertex) // if per vertex culling is enabled
						{
							if(PointOnCamera(pos)) // if this vertex is on the camera
							{
								...
								if(delta >= particleTime) 
								{
									ps.Emit(pos,Vector3.zero,particleSize,particleLifetime,ps.startColor);
		...
		IEnumerator Wave()
		{
			...
			int range = (int)(verticies.Length * waveEffectRange);
			...
			ParticleSystem ps = particleSystem.GetComponent<ParticleSystem> (); // get the particle system
			ps.playOnAwake = false;
